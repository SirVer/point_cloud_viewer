...
impl point_viewer_prost::grpc::proto::server::Octree for OctreeService {
     ...
     fn get_node_data(&mut self, request: Request<GetNodeDataRequest>) -> Self::GetNodeDataFuture {
         ...
         let data = self.state.octree
             ...
             .unwrap();
         let node_proto = point_viewer::proto::Node::new();
...
fn to_prost(node: &point_viewer::proto::Node) -> point_viewer_prost::proto::Node {
    point_viewer_prost::proto::Node {
        ...
        id: Some(point_viewer_prost::proto::NodeId {
            level: node.id.unwrap().level,
            index: node.id.unwrap().index,
        }),
    }
}
...
fn main() {
        ...
        point_viewer_prost::proto::Meta {
            ...
            bounding_box: Some(point_viewer_prost::proto::AxisAlignedCuboid {
                min: Some(point_viewer_prost::proto::Vector3f {
                    x: other.bounding_box.unwrap().min.unwrap().x,
                    y: other.bounding_box.unwrap().min.unwrap().y,
                    z: other.bounding_box.unwrap().min.unwrap().z,
                }),
                max: Some(point_viewer_prost::proto::Vector3f {
                    x: other.bounding_box.unwrap().max.unwrap().x,
                    y: other.bounding_box.unwrap().max.unwrap().y,
                    z: other.bounding_box.unwrap().max.unwrap().z,
                }),
            }),
    ...
    let handler = OctreeService {
         state: Arc::new(State { meta, octree }),
    };

    // let new_service = server::Octree::<
        // GetMetaFuture=<OctreeService as server::Octree>::GetMetaFuture,
        // GetNodeDataFuture=<OctreeService as server::Octree>::GetNodeDataFuture,
        // GetPointsInBoxStream=<OctreeService as server::Octree>::GetPointsInBoxStream,
        // GetPointsInBoxFuture=<OctreeService as server::Octree>::GetPointsInBoxFuture,
        // >::new(handler);
    let new_service = server::Octree::new(handler);
